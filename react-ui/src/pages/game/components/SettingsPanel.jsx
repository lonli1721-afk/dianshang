...
const SETTINGS_FIELDS = [
  ...
  { key: 'game_gemini_api_key', label: 'Gemini API Key（AI文本分析/生图）', desc: '用于电商素材平台的AI提示词分析和Gemini图像生成', color: '#10b981' },
  { key: 'game_ark_api_key', label: 'ARK API Key（即梦Seedance/Seedream）', desc: '用于电商素材平台的Seedance视频生成和Seedream图片生成', color: '#f59e0b' },
  { key: 'game_vidu_api_key', label: 'VIDU API Key（VIDU视频生成）', desc: '用于电商素材平台的VIDU Q3视频生成', color: '#8b5cf6' },
  { key: 'game_dashscope_api_key', label: 'DashScope API Key（万相 / HappyHorse）', desc: '用于阿里云万相视频换人，以及 HappyHorse 视频生成与编辑', color: '#3b82f6' },
  { key: 'game_toapis_api_key', label: 'ToAPIs API Key（Image2 / Veo 3.1）', desc: '用于 ToAPIs 的 Image2/GPT Image 2 生图，以及 veo3.1-fast / veo3.1-lite / veo3.1-quality 视频生成', color: '#14b8a6' },
  ...
  return (
    <div style={{ padding: 32, maxWidth: 700, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>电商素材 API 设置</h1>
      </div>
      <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>
        配置电商素材平台专用的 API Key，与主应用独立。如未配置则自动使用主应用的 Key。
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {SETTINGS_FIELDS.map(({ key, label, desc, color, multiline, plain }) => (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ flex: 1, position: 'relative' }}>
                {multiline ? (
                  ...
                  <input
                    id={`game-setting-input-${key}`}
                    type={plain || showKeys[key] ? 'text' : 'password'}
                    value={settingInputs[key] || ''}
                    onChange={event => onInputChange(key, event.target.value)}
